...
bool rtabmap_msgs__msg__map_graph__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("rtabmap_msgs.msg._map_graph.MapGraph", full_classname_dest, 36) == 0);
  }
  rtabmap_msgs__msg__MapGraph * ros_message = _ros_message;
  {  // header
  ...
  {  // links
    ...
    if (!rtabmap_msgs__msg__Link__Sequence__init(&(ros_message->links), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create rtabmap_msgs__msg__Link__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    rtabmap_msgs__msg__Link * dest = ros_message->links.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!rtabmap_msgs__msg__link__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * rtabmap_msgs__msg__map_graph__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("rtabmap_msgs.msg._map_graph");
  ...
  {  // links
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = rtabmap_msgs__msg__link__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
